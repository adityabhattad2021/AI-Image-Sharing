    ...
    return (
        ...
        <nav className={`ease-in-out duration-300 md:m-7 px-4 md:rounded-3xl ${!props.isCreate && "sticky top-0 md:top-2 z-10 bg-black backdrop-filter backdrop-blur-xl"} ${props.isCreate ? "bg-opacity-20" : "bg-opacity-30"}  md:border-black ${!props.isCreate && "shadow-[rgba(_0,_0,_0,_0.9)_0px_9px_15px]"}`}>
            ...
            <div className={`sm:hidden absolute m-5 p-4 flex flex-col gap-3 rounded-2xl top-[105px] right-0 left-0  bg-black backdrop-filter backdrop-blur-lg bg-opacity-30 z-10  py-4 ${!toggleDrawer ? "-translate-y-[100vw]" : "translate-y-0"
                ...
                {isSignedIn && <><div className="ease-in-out duration-300  flex justify-center backdrop-blur-lg hover:text-black bg-black font-semibold hover:bg-white  text-white font-Nota py-2 px-4  border-black cursor-pointer hover:border-transparent rounded-[20px] text-xl">{isSignedIn && <Link href="/user/collection">Your Collection</Link>}</div>
