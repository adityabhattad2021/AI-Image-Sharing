...
const Create = () => {
    ...
    const { mutate:getPrpmpt, isLoading: isGettingPrompt } = api.images.generatePrompt.useMutation({
        onSuccess: (data) => {
            if(!data){
        ...
        <div className="w-[93%] md:w-[95%] overflow-hidden h-screen  flex flex-col md:justify-center items-center gap-6 mx-4 mt-[80px] md:my-auto relative">
            ...
            <div className="flex flex-col mt-4 w-full md:w-[900px] gap-6">
                <div className="w-full flex flex-col md:flex-row gap-2">
                    <input className="placeholder-black duration-400 ease-in-out w-full md:w-[75%] h-[60px] font-Nota font-bold backdrop-filter backdrop-blur-lg bg-opacity-30 bg-black focus:ring-white focus:border-white focus:shadow-lg p-4 rounded-[20px]" value={what} onChange={(e) => setWhat(e.target.value)} placeholder="What do you want to create?" />
                    {
                        !isMobile && (
                            <>
                                ...
                                <button className="ease-in-out duration-300 w-full md:w-[7%] h-[60px] rounded-[20px] bg-[#00A67E] hover:bg-green-400 mt-4 md:mt-0 flex justify-center items-center " onClick={() => getPrpmpt({ what: what })}><Image src={logo} alt="OpenAI Icon" width={45} height={45} /></button>
                            </>
                        )
                    }
                ...
                <div className="w-full">
                    <textarea name="detailedDescription" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe as precisely as possible..." cols={30} rows={6} className="w-full rounded-[20px] placeholder-black transition-400 ease-in-out font-Nota font-bold backdrop-filter backdrop-blur-lg bg-opacity-30 bg-black focus:ring-white focus:border-white focus:shadow-lg p-4"></textarea>
                </div>
                <div className="w-full h-[70px] md:h-[350px] rounded-[30px] duration-400 ease-in-out flex justify-center items-center font-Nota font-bold backdrop-filter backdrop-blur-lg bg-opacity-30 bg-black focus:ring-white focus:border-white focus:shadow-lg p-4">
